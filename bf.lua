...
local function AntiBan()
    ...
    -- Fungsi aman untuk menghancurkan objek hanya jika ada
    ...
    -- Destroy scripts only when necessary to avoid continuous looping
    for location, script_names in pairs(scripts_to_destroy) do
        ...
        if target then
            -- Ambil turunan hanya jika ada perubahan yang terdeteksi
...
local function AntiCheatDetector()
    -- Optimized to avoid performance hit
    local mt = getrawmetatable(game)
    local old = mt.__namecall
    if not mt.__namecall_optimized then
        setreadonly(mt, false)
        mt.__namecall = newcclosure(function(...)
            local args = {...}
            local method = getnamecallmethod()
            if method == "FireServer" or method == "InvokeServer" then
                if tostring(args[1]):find("BanRemote") or tostring(args[1]):find("AntiCheat") or tostring(args[1]):find("ReportPlayer") or tostring(args[1]):find("DetectUser") then
                    return -- Just prevent the action instead of causing a wait which can lead to lag
                end
            end
            return old(...)
        end)
        mt.__namecall_optimized = true -- Flag to prevent overriding multiple times
        setreadonly(mt, true)
    end
end

-- Fungsi utama untuk menjalankan semua proteksi
local function MainProtection()
    -- Hanya jalankan sekali, jangan panggil terus-menerus
    AntiBan()
    AntiCheatDetector()
end

-- Jalankan proteksi utama
MainProtection()
...
Main:AddToggle("Aura Attack Npc",true,function(value)
        _G.FastAttack = value
    end)      
...
STg:AddToggle("Esp Mob", false, function(nec)
    ...
    STg:AddToggle("Gun Skill Z",true,function(value)
        _G.GunSkillZ = value
    end)

    STg:AddToggle("Skill Gun X",true,function(value)
        _G.GunSkillX = value
    end)

    _G.AutoMasteryGunSkill = true
	spawn(function()
		while task.wait() do
			pcall(function()
				if _G.UseSkill and _G.AutoMasteryGunSkill then
					if _G.SkillZ then
...
RaceV4:AddToggle("Auto Train & Buy Gear",_G.Auto_Bone,function(value)
  ...
  game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer('UpgradeRace','Buy')
    ...
    local PlayerSelectAimbot = P:AddDropdown("Select Player",Playerslist,function(value)
        PlayerSelectAimbot = value
    end)
    
    P:AddButton("Refresh Player",function()
        Playerslist = {}
        PlayerSelectAimbot:Clear()
        for i,v in pairs(game:GetService("Players"):GetChildren()) do  
            PlayerSelectAimbot:Add(v.Name)
        end
    end)
    P:AddToggle("Target Lock",_G.LockTarget, function(value)
        _G.LockTarget = value
    end)
            ...
            if _G.EliteCheck then
                ...
                for _, npcName in pairs(eliteNPCs) do
                    ...
                    if npcInReplicated or npcInWorkspace then
                        local notification = require(game:GetService("ReplicatedStorage").Notification).new("Elite Hunter " .. npcName .. " has spawned"):Display()
                            wait()
                            setthreadcontext(5)
                        break
                    end
...
Mh:AddToggle("Hop Server Rip Indra Or Low Server", _G.HopFindRipIndra, function(value)
...
spawn(function()
    ...
    Mh:AddButton("Visual Space B-Team", function()
        pcall(function()
            require(game.ReplicatedStorage.Notification).new("<Color=Red> 1<Color=/>"):Display()
            wait(1)
            require(game.ReplicatedStorage.Notification).new("<Color=Red> 2<Color=/>"):Display()
